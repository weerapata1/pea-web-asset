...
public class EquipmentController {
    ...
    public EquipmentController(EquipmentRepository equipmentRepository, DeviceRepository deviceRepository,
            DeviceTypeRepository deviceTypeRepository, EquipmentLocationRepository equipmentLocationRepository) {
    ...
    public ResponseEntity getQuantityOfEquipment(@PathVariable("dType") String dType) {
        // เช็ดประเภทอุปกรณ์ที่คงเหลือใน stock พร้องจ่าย โดยตึงค่าเป็นประเภท
        // และสามารถดูได้ว่ามีอะไรอยู่บ้าง S/N ไหน
        try {
    ...
    public ResponseEntity addEquipment(@RequestParam("file") MultipartFile file, @RequestParam("dType") Long dType,
            @RequestParam("locId") Long locId) throws Exception {
        try {
